...
# Ok, this is where the parsnip shit comes into play (linear_reg is a parsnip thing)
spec_lin_reg <- linear_reg()
spec_lin_reg

# Only certain engines are supported. lm and glmnet, and stan which is bayesian(?)
spec_lm <- set_engine(spec_lin_reg, "lm")
spec_lm

fit_lm <- fit(
  spec_lm, #--the type of analysis and engine
  log10(Sale_Price) ~ Longitude + Latitude, #--the formula
  data = ames_train #--the data
)
...
# you can use stupid notation too, 
...
#This is just looking at how we might incorporate folds
...
cv_splits$splits[[1]] %>% analysis() %>% dim() #--here's our 1977
cv_splits$splits[[1]] %>% assessment() %>% dim() #--and here's our 222
...
fit_model <- function(mysplit, myspec) {
  fit(
    object = myspec, 
    formula = geo_form,
    data = analysis(mysplit) # <- pull out training set
  )
}

# For each resample, call fit_model()
cv_splits <- cv_splits %>% 
  mutate(mymodels_lm = map(splits, fit_model, spec_lm))

cv_splits

# Slide 36 -------------------------------------------------------

compute_pred <- function(mysplit, mymodel) {
  
  # Extract the assessment set
  assess <- assessment(mysplit) %>%
    mutate(Sale_Price_Log = log10(Sale_Price))
  
  # Compute predictions (a df is returned)
  pred <- predict(mymodel, new_data = assess)
  
  bind_cols(assess, pred)
}

cv_splits <- cv_splits %>%
  mutate(mypred_lm = map2(splits, mymodels_lm, compute_pred))
# map2 takes in 2 column-based arguments for one function
...
compute_perf <- function(pred_df) {
  ...
  # NOTE: this uses yardstick package
  numeric_metrics <- metric_set(rmse, rsq) #--now numeric metrics is a function
...
cv_splits <- cv_splits %>%
  mutate(myperf_lm = map(mypred_lm, compute_perf))

cv_splits

# Slide 38 -------------------------------------------------------

# How do we get the stats out of that buried column?
cv_splits$myperf_lm[[1]]
...
# With this, we can plot resids vs predictors to see if we missed something
holdout_results <- 
  cv_splits %>%
  unnest(mypred_lm) %>%
