...
# what the fuck are bsplines?

# Slide 38 -------------------------------------------------------

# This is what bsplines are
ggplot(ames_train, 
...
# Slide 40 -------------------------------------------------------
# use rsample to make 10 splits
# prep works on data, but resamples aren't dataframes
# prepper takes care of that

cv_splits <- cv_splits %>% 
  mutate(nonlin_terms = map(splits, prepper, recipe = nonlin_terms))

# so each split has it's recipe
cv_splits$nonlin_terms[[1]]
# look at what it would produce
...
# build a helper function w/mutate map strategy
...
# make sure to bake the assessment set
# I'm still confused about bake vs juice

# another helper function, need data, recipe, and model
# uh oh. map takes 1 arg, map2 takes 2, but you need pmap to do more....
...
# warnings show you might be extrapolating outside where you trained
...
cv_splits %>%
  unnest(pred) %>%
  group_by(id) %>%
  metrics(truth = Sale_Price, estimate = .pred) %>% #--this is yardstick
  group_by(.metric) %>%
  summarise(
    resampled_estimate = mean(.estimate)
  )
# wow so this is better than knn

# Slide 45 -------------------------------------------------------

# look at preds vs sale price 
...
# whoa there are some outliers, what's the problem?
# well, there are 5 houses that were sold by a dying business
# the model helped identify those, actually. neat.
# what other things can you look at?

# 1. preds vs true values (which we just did)
# 2. resids vs individual preditors
# 3. those hat values
